...
I used a Random Forest Classifier and chose the best hyperparameters using Grid Search CV with 5-fold cross-validation. I one hot encoded the categorical variables: `'MONTH'`, `'CLIMATE.REGION'`, and `'CAUSE.CATEGORY'`. I also performed a log-transformation on  `'AREAPCT_URBAN'` and `'TOTAL.SALES'` as both of their distributions were extremely skewed. I passed the remaining columns through.
